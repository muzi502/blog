...
去年的时候听了 Rancher 的直播分享讲  [k3s 在边缘计算中的应用实践](http://dockone.io/article/9384)，就感觉很好玩儿，当时对 K3s 还是很感兴趣，无奈当时手头没有合适的开发板可玩，就没愿意花钱再买块树莓派吃灰 😂。直到上周末的时候一次机会得以重新认识了一下 [K3s](https://github.com/rancher/k3s) 所以就想找个设备来玩玩 K3s。于是东挑西选就选定了斐讯 N1 ，优点多多，不仅可以做路由器还能跑 docker 、还能做 NAS 等等，两个字真香 😂。之前我一直使用 `R6300V2` 刷了梅林以及另一台 `WNDR3700V4` 刷了 `OpenWrt` 做透明代理，在 `ESXi` 上也装了个 `OpenWrt` 软路由给内网的虚拟机做透明代理，但还是心有不甘想找一台性能更好一点的设备来做旁路网关。综上就种草买了早已经被大家玩烂了的 [N1 盒子](https://pockies.github.io/2019/03/07/phicomm-n1/)。

## 开箱

### 外包装

![](https://p.k8s.li/20200308170852893.png)

### 接口

![](https://p.k8s.li/20200308170909549.png)

### 开机画面

![](https://p.k8s.li/20200308170935704.png)

### 电视系统

![](https://p.k8s.li/20200308171000114.png)

### 配置

#### 拆机

![](https://p.k8s.li/20200309141919253.png)
...
![](https://p.k8s.li/20200308180733757.png)
...
> Openwrt_U 盘直刷包：链接：[https://pan.baidu.com/s/1K0bNItsY1-Br4o1EsRokkg](https://pan.baidu.com/s/1K0bNItsY1-Br4o1EsRokkg) 提取码：lidf
...
- [Pi&N1 交流群通知频道](https://t.me/NewPiN1Channel)
- [山小炮网盘](https://disk.longe.xyz/)
- [Phicomm N1 刷 openwrt 路由固件详细笔记.pdf](https://disk.longe.xyz/N1/OpenWrt/%E3%80%902019-6-20%E3%80%91Phicomm%20N1%E5%88%B7openwrt%E8%B7%AF%E7%94%B1%E5%9B%BA%E4%BB%B6%E8%AF%A6%E7%BB%86%E7%AC%94%E8%AE%B0.pdf)
- [28+o 版 N1_OP_U 盘直刷包](https://disk.longe.xyz/N1/OpenWrt/28%2Bo%E7%89%88N1_OP_U%E7%9B%98%E7%9B%B4%E5%88%B7%E5%8C%85/)
- [[2020-3-8]★29+ 版 N1_OP_U 盘直刷包，及贝壳云_OP_线刷包，内核 5.4](https://www.right.com.cn/forum/thread-981406-1-1.html)

## 刷机

### 开启 ADB

![](https://p.k8s.li/20200309085941030.png)
...
> PS：提到 adb 咱就不由自主地想起了去年世界五百强他家的号称要取代 Android 、自主研发、微内核、面向确定[时延](https://zh.wikipedia.org/wiki/%E6%97%B6%E5%BB%B6)、分布式、形式化验证、多场景；大名鼎鼎的哄懵（PPT）操作系统竟然也有 ADB （偷笑

在这里需要注意的是，我使用网线连接盒子和路由器后，屏幕上显示的 IP 并不是我本地内网的 IP ，这一点很奇怪，而使用无线连接到我的无线路由器后去能获取到正常的内网 IP ，所以如果你使用网线连接时也遇到这样的情况，不妨尝试一下使用无线连接。

![](https://p.k8s.li/20200308171341327.png)
...
![](https://p.k8s.li/20200308152333262.png)
...
不过意外发现该盒子的 USB 口竟然支持使用 echo 命令来修改 `/sys/bus/usb/devices/usb1/power/level` 的值来控制 USB 的电源状态，这样从 USB 口引出正负极接在继电器上，这就是一个通过网络控制的开关。我在 `R6300V2` 和 `WNDR3700V4` 上梦寐以求没有的功能居然在这个破盒子上有。意外收获！
